...
def get_wide_region(region):
    '''
    Получает более широкий регион. 
    На входе значение вида "Петуховский |Курганская область",
    на выходе возвращает "Курганская область".
    '''
    wide_region = 
    return wide_region
    
def get_region(range4, conn):
    sql = '''
    ...
    res = cur.fetchall()
    if len(res) > 1:
        print "For {r_4} there are several regions:".format(r_4=range4)
        for tup in res:
            print tup[0]
        region = get_wide_region(res[0][0])
...
def get_short_ranges(full_ranges):
    ...
    for range4 in lines:
        ...
        if is10in_list(ranges4) is True:
            ...
            short_range.append(get_region(range4, conn))
